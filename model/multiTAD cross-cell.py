...
class Net_inter(nn.Module):
    def __init__(self):
        ...
        self.cnn_block = nn.Sequential(nn.Conv1d(in_channels =numberr, 
                                                 out_channels = 64, 
        ...
        self.channel_attention = ChannelAttention(numberr)
    def forward(self, x):
        x = x.permute(0, 2, 1)
...
def main(cell_line, inter_file, mid_file, nb_file,inter_test,mid_test, nb_test):
    '''
    load data
    '''
    ...
    ##测试细胞系
    (xb1,yb1) = extract(inter_test,1)
    (xb2,yb2) = extract(inter_test,0)
    xb_inter = np.concatenate((xb1,xb2))
    yb_inter = np.concatenate((yb1,yb2))
    
    (xb1,yb1) = extract(mid_test,1)
    (xb2,yb2) = extract(mid_test,0)
    xb_mid = np.concatenate((xb1,xb2))
    yb_mid = np.concatenate((yb1,yb2))
    
    (xb1,yb1) = extract(nb_test,1)
    (xb2,yb2) = extract(nb_test,0)
    xb_nb = np.concatenate((xb1,xb2))
    yb_nb = np.concatenate((yb1,yb2))
    ...
    x_train_inter, x_val_inter, y_train_inter, y_val_inter = train_test_split(x_inter, y_inter,train_size=0.9, random_state=49)
                                                                                
    x_train_nb, x_val_nb, y_train_nb, y_val_nb = train_test_split(x_nb, y_nb, train_size=0.9, random_state=49)
  
    x_train_mid, x_val_mid, y_train_mid, y_val_mid = train_test_split(x_mid, y_mid, train_size=0.9, random_state=49)
    ...
    xb_inter = torch.from_numpy(xb_inter).float()
    xb_mid = torch.from_numpy(xb_mid).float()
    xb_nb= torch.from_numpy(xb_nb).float()
    y_test= torch.from_numpy(yb_nb).long()
    ...
    test_set = MyDataset(xb_inter, xb_mid, xb_nb,y_test)
...
if __name__ == '__main__': 
    ...
    cell_lines=["Hesc","GM12878","IMR90","K562"]
    ...
    for i in range(4):
        ...
        cell_line2s = [cell for index, cell in enumerate(cell_lines) if index !=i]
        print(cell_line2s)
        for cell_line2 in cell_line2s:
            for j in range(10):
                name=str(size1[0])+"kb"
                names=name
                names=name
                ...
                inter_test=file_dir+"{0}/".format(cell_line2)+"3d5_"+name+"/"
                name=str(size2[0])+"kb"
                names+=","+name
                mid_file = file_dir+"{0}/".format(cell_line)+"3d5_"+name+"/"
                mid_test = file_dir+"{0}/".format(cell_line2)+"3d5_"+name+"/"
                name=str(size3[0])+"kb"
                names+=","+name
                nb_file = file_dir+"{0}/".format(cell_line)+"3d5_"+name+"/"
                nb_test = file_dir+"{0}/".format(cell_line2)+"3d5_"+name+"/"
                sizes=str(j+1)
                y_pred, y_test,AUC,ACC,AUPR,test_loss,precision,recall,fmeasure = main(cell_line,inter_file,mid_file,nb_file,inter_test,mid_test,nb_test)
